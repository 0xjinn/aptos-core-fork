...
pub async fn connect_to_remote(
    ...
    // Create the QUIC server endpoint. This will call bind on 127.0.0.1.
...
mod test {
    ...
    async fn simple_listen_and_dial() -> Result<(), ::std::io::Error> {
        let mut t = QuicTransport::new().and_then(|mut out, addr, origin| async move {
            ...
            match origin {
                ConnectionOrigin::Inbound => {
                    ...
                    println!(
                        "(simple_listen_and_dial: {:?}, {:?}) Verifying third data packet: {:?}",
                        origin, addr, buf
                    );
                    assert_eq!(&buf, b"Air");
                ...
                ConnectionOrigin::Outbound => {
                    ...
                    println!(
                        "(simple_listen_and_dial: {:?}, {:?}) Verifying first data packet: {:?}",
                        origin, addr, buf
                    );
                    ...
                    println!(
                        "(simple_listen_and_dial: {:?}, {:?}) Verifying second data packet: {:?}",
                        origin, addr, buf
                    );
                    assert_eq!(&buf, b"Fire");
        ...
        let (outgoing, _incoming) = timeout(Duration::from_secs(30), join(dial, listener))
            .await
            .unwrap();
