...
impl CliCommand<()> for RunLocalTestnet {
    ...
    async fn execute(mut self) -> CliTypedResult<()> {
        ...
        // see `ShutdownStep` for more info. In particular, so ctrl-c happens more or
        // less instantly, we only register this handler after all the services have
        // started.
        ...
        // Because we added the ctrl_c task last, we can figure out if that was the one
